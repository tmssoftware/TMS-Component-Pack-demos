...
begin
  ...
  if Pos('Custom', Result) > 0 then
    Result := '$' + IntToHex(Integer(ColorToRGB(fEdit.SelectColor)), 6);
end;

procedure TJPMEColorComboBoxEditLink.SetEditorValue(s: string);
begin
  if Pos('$', s) > 0 then
    fEdit.ItemIndex := fEdit.Items.IndexOf('Custom Color...')
  else
    if (s <> '') then fEdit.ItemIndex := fEdit.Items.IndexOf(s);
end;

procedure TJPMEColorComboBoxEditLink.SetProperties;
begin
  inherited;
  fEdit.ShowCustomColor := ShowCustomColor;
  fEdit.ShowSystemColors := ShowSystemColors;
end;
...
procedure Register;
begin
  RegisterComponents('TMS EditLinks', [TJPMEFontComboBoxEditLink,
