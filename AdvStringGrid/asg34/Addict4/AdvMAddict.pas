unit AdvMAddict; // Updated for Addict 4
...
begin
     ...
     Word := FParsingEngine.NextWord(FStringParser);
...
procedure TAdvMemoAddictChecker.CheckWord(LineNo, LinePos: Integer; s: string);
begin
     if not Assigned(Memo) then
          Exit;
...
procedure TAdvMemoAddictChecker.CorrectWord(LineNo, LinePos: Integer; var s: string);
var
     Repl: string;
begin
     if not Assigned(Memo) then
...
procedure TAdvMemoAddictChecker.Notification(AComponent: TComponent; AOperation: TOperation);
begin
     inherited;
